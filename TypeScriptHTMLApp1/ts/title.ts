...
class Title extends KYScene implements IScene {
    ...
    public calc = (): void => {
        ...
        if (this.count >= 60 && this.decidePoint == -1) {
        ...
        if (this.decidePoint != -1) {
            this.whiteDepth = (this.count - this.decidePoint) / 30;
            this.music.changeVolume("bgm0", 1 - (this.count - this.decidePoint) / 30);
            if ((this.count - this.decidePoint) == 30) {
    ...
    public graph = (): void => {
        if (this.count < 60) {
            ...
            if (this.menuSelect % 2 == 0)
                this.image.draw(this.ctx, (this.decidePoint == -1 || ((this.count - this.decidePoint) % 8) >= 4) ? "title01_b" : "title01_by", 86, 266);
            else
                this.image.draw(this.ctx, "title01_s", 102, 274);
            if (this.menuSelect % 2 == 1)
                this.image.draw(this.ctx, (this.decidePoint == -1 || ((this.count - this.decidePoint) % 8) >= 4) ? "title02_b" : "title02_by", 266, 266);
            else
